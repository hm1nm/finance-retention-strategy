...
def plot_feature_importance_rf(model, feature_names, title):
    ...
    plt.title(f'Feature Importance (Top 20) - {title}')
    plt.tight_layout()
    plt.show()
...
def visualize_shap_summary(model, X_train, top_n_shap=20):
    ...
    # RandomForest도 TreeExplainer로 가능하지만 속도가 느릴 수 있음
    # summary_plot을 그리기 위해 TreeExplainer 사용
    explainer = shap.TreeExplainer(model)
    
    # RandomForest의 경우 shap_values는 리스트 형태 [class0_shap, class1_shap]일 가능성 높음
    # (주의) scikit-learn RandomForest의 경우 check_additivity=False 옵션이 필요할 수도 있음.
    # 또한 TreeExplainer는 모델 객체 자체를 받음.
    
    try:
        shap_values = explainer.shap_values(X_shap, check_additivity=False)
    except Exception as e:
        print(f"SHAP calculation warning/error: {e}")
        # KernelExplainer 대안 시도 가능하나 너무 느림. 여기서는 스킵하거나 알림.
        return
...
# ========================================================================================
# 5. Model Training Modules (RandomForest)
# ========================================================================================

def train_eval_randomforest_main(df_train, target_col='Target'):
    print(f"\n🏋️ [Model Training] RandomForest...")
    ...
    plot_confusion_matrix_heatmap(y_test, y_pred, "RandomForest")
    
    # Feature Importance (pass feature names)
    plot_feature_importance_rf(model, X_train.columns, "RandomForest")
...
if __name__ == "__main__":
    print("🚀 Starting Feature Selection & Visualization Pipeline (RandomForest Version)...")
    ...
    if os.path.exists(DATA_FILE_PATH):
        ...
        if df_final is not None:
            ...
            # Execute RandomForest Training and Evaluation
            rf_model, X_train_rf = train_eval_randomforest_main(df_final)
            
            print("✅ Pipeline (RandomForest) Completed Successfully.")
    else:
